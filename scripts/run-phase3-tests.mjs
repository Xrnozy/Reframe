...
if (!npmCli) throw new Error("npm_execpath is required; run through `npm run test:phase3`");
...
async function run(script) {
  ...
  process.stdout.write(`PHASE3_COMMAND ${JSON.stringify({ script, exitCode: code, runtimeMs: Number((performance.now() - started).toFixed(2)) })}\n`);
  return code;
}
...
for (const script of [
  ...
  "test:phase3:e2e",
  "test:phase3:performance",
  "test:phase2:integration",
  "test:phase2:performance",
  "test:phase1:integration",
  "test:phase1:performance",
  "test:unit",
  "test:phase0:integration",
  "test:phase0:performance",
  "test:e2e",
  "verify:phase1-leaks",
  "verify:phase2-leaks",
  "verify:phase3-leaks",
