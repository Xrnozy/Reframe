...
if (!npmCli) throw new Error("npm_execpath is required; run through `npm run test:phase4`");
...
async function run(script) {
  ...
  process.stdout.write(`PHASE4_COMMAND ${JSON.stringify({ script, exitCode: code, runtimeMs: Number((performance.now() - started).toFixed(2)) })}\n`);
  return code;
}
...
for (const script of [
  ...
  "test:phase4:regression:ws",
  "test:phase4:regression:e2e",
  "test:phase4:regression:cli",
  "test:phase4:regression:pack",
]) {
  if (await run(script)) process.exit(1);
}
const after = await digest(protectedTrees);
process.stdout.write(`PHASE4_FIXTURE_DIGEST ${JSON.stringify({ before, after, unchanged: before === after })}\n`);
