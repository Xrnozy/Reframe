...
if (!npmCli) throw new Error("npm_execpath is required; run through `npm run test:phase7`");
...
async function run(script) {
  ...
  process.stdout.write(`PHASE7_COMMAND ${JSON.stringify({ script, exitCode: code, runtimeMs: Number((performance.now() - started).toFixed(2)) })}\n`);
  return code;
}

const protectedTrees = [path.join(root, "demo"), path.join(root, "tests", "fixtures", "templates")];
const before = await digest(protectedTrees);
for (const script of ["build", "test:phase7:integration", "test:phase7:e2e", "test:phase7:performance", "test:phase7:regression"]) {
  if (await run(script)) process.exit(1);
}
const after = await digest(protectedTrees);
process.stdout.write(`PHASE7_FIXTURE_DIGEST ${JSON.stringify({ before, after, unchanged: before === after })}\n`);
